...
export default function App() {
  ...
  useEffect(() => {
    ...
    setHero(h => {
      if (!h || h.visited?.includes(key)) return h
  ...
  function resolveTarget(rawTarget) {
    ...
    // Règle anti-retour : sauf cas explicite de menu/sauvegarde, un choix narratif ne renvoie plus vers un passage déjà lu.
    // Si une branche converge par erreur sur un ancien passage, on avance vers le premier passage non lu du livre.
    if (target <= pid || visited.includes(`${bookId}:${target}`)) {
      for (let candidate = Math.max(pid + 1, target + 1); candidate <= 300; candidate++) {
  ...
  function go(goto, eff) {
    ...
    if (target !== goto) addLog('Transition corrigée : l’aventure évite un passage déjà lu.')
    setPid(target)
    setCombat(null)
    setLastRoll(null)
